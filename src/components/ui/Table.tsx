                    ...
                    {table.getHeaderGroups().map((headerGroup) => (
                        <tr key={headerGroup.id} className="border-b border-border-card">
                            {headerGroup.headers.map((header) => {
                                return (
                                    <th key={header.id} className="text-left px-md py-md text-[11px] font-semibold uppercase tracking-wider text-muted" >
                ...
                <tbody>
                    {table.getRowModel().rows?.length ? (
                        table.getRowModel().rows.map((row) => (
                            <tr key={row.id} className="border-b border-border-card last:border-0 hover:bg-surface-container-low/50 transition-colors" >
